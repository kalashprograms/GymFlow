...
apiRouter.post('/ai-assistant/chat', async (req: Request, res: Response) => {
  ...
  const gym = db.getGym(gymId);
  const members = db.getMembers(gymId);
  const stats = db.getDashboardStats(gymId);

  try {
    const ai = getGeminiClient();
    if (!ai) {
      return res.json({
        reply: `**GymFlow Smart Assistant**\n\nI'm ready to assist with membership retention, messaging scripts, and gym operations. (Note: Add GEMINI_API_KEY for dynamic generative intelligence)\n\nHere are quick recommendations for **${gym?.name || 'your gym'}**:\n\n- **Expiring Today:** ${stats.expiringToday} member(s) need immediate renewal follow-up.\n- **This Week:** ${stats.expiringThisWeek} member(s) expiring within 7 days. Send standard 10% renewal incentive.\n- **Pending Payments:** $${stats.pendingPayments} can be collected via UPI/Card link.`,
      });
    }

    const systemContext = `You are the AI Business & Retention Copilot for GymFlow, assisting gym owner "${gym?.ownerName}" at "${gym?.name}".
Current Gym Summary:
...
- Pending Collections: $${stats.pendingPayments}

Respond with actionable, professional, concise advice for gym management, member retention strategies, WhatsApp/SMS renewal notification scripts, or workout/diet templates as requested.`;
    ...
    return res.status(500).json({
      ...
      details: err.message || 'Internal AI service error',
    });
  }
});
